...
class BulletproofMeetingMinutesGenerator:
    ...
    def safe_api_call(self, func, *args, **kwargs):
        ...
        for config in retry_configs:
            for attempt in range(config["max_retries"]):
                try:
                    ...
                    is_last_attempt = (config is retry_configs[-1]) and (attempt == config["max_retries"] - 1)
                    if is_last_attempt:
                        break
        ...
        return False, "", last_error or "Unknown API error"
...
- """
    ...
    def process_meeting_bulletproof(self, json_file: str, output_file: str = 'final_minutes.txt') -> str:
        ...
        segments, _complete_transcript = self.load_and_structure_data(json_file)
